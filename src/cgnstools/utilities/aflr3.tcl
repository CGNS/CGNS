...
proc aflr3_import {w name exe} {
  ...
  toplevel $w
  wm title $w "AFLR3/FAST File Import"
  wm transient $w .
  wm protocol $w WM_DELETE_WINDOW {set Import(done) 0}

  FrameCreate $w.input -text "AFLR3/FAST Input" -font $Font(bold)
  ...
  FrameCreate $w.type -text "File Type" -font $Font(bold)
  pack $w.type -side top -padx 5 -pady 2 -fill x
  set type [FrameGet $w.type]

  radiobutton $type.a -text "AFLR3 file" \
    -variable Import(aflr3,type) -value A
  radiobutton $type.f -text "FAST file" \
    -variable Import(aflr3,type) -value F
  pack $type.a $type.f -side left -expand 1
  ...
  if {[import_buttons $w aflr3_import_check]} {
    if {$Import(aflr3,type) != "A"} {
      lappend cmd -$Import(aflr3,type)
    }
    ...
    import_run "AFLR3/FAST Import" $cmd $Import(cgnsfile)
  }
}
...
proc aflr3_browse {w} {
  ...
  set fname [FileOpen "AFLR3/FAST File" $Import(aflr3file) $w \
    {{{AFLR3/FAST Files} {.ugrid .fgrid}} {{All Files} {*}}}]
  if {$fname != ""} {
    ...
    if {[file extension $fname] == ".fgrid"} {
      set Import(aflr3,type) F
    } else {
      set Import(aflr3,type) A
    }
...
#  if {$cmd == ""} return
  ...
  if {[export_buttons $w aflr3_export_check]} {
    ...
    lappend cmd -$Export(aflr3,endian) -$Export(aflr,prec)
