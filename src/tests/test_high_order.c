...
/* write QUAD element connectivity for inflow face (user can give any name) */
     ...
     * that is absent look identical from the outside.
     * ------------------------------------------------------------------- */
    printf ("writing test_ho_rind.cgns (Rind under InterpolationPoints) ...\n");
    {
        ...
        /* Family + degree-2 Lagrange basis, bound to the zone, so the block is
         * otherwise well formed and Rind is the only thing wrong with it. */
        {
            int famr, sir;
            double pu[9] = {-1.,0.,1.,-1.,0.,1.,-1.,0.,1.};
            double pv[9] = {-1.,-1.,-1.,0.,0.,0.,1.,1.,1.};
            if (cg_family_write (fr, br, "RindFam", &famr) ||
        ...
        if (cg_sol_write (fr, br, zr, "FS", CGNS_ENUMV(InterpolationPoints), &sr) ||
