...
/* write QUAD element connectivity for inflow face (user can give any name) */
    ...
    /* ---------------------------------------------------------------------
     * CPEX-0045: Rind_t has no meaning under GridLocation=InterpolationPoints.
     * Write one deliberately so cgnscheck has a file to flag; a rule with no
     * negative test proves nothing, since a check that never fires and a check
     * that is absent look identical from outside.
     * ------------------------------------------------------------------- */
    printf ("writing test_ho_rind.cgns (Rind under InterpolationPoints) ...\n");
    {
        int fr, br, zr, sr, fld, secr, ci, famr, sir, k;
        ...
        if (cg_open ("test_ho_rind.cgns", CG_MODE_WRITE, &fr) ||
            cg_base_write (fr, "Base", 2, 2, &br) ||
            cg_zone_write (fr, br, "Zone", sz, CGNS_ENUMV(Unstructured), &zr) ||
            cg_coord_write (fr, br, zr, CGNS_ENUMV(RealDouble), "CoordinateX", xyz, &ci) ||
            cg_coord_write (fr, br, zr, CGNS_ENUMV(RealDouble), "CoordinateY", xyz, &ci) ||
            cg_section_write (fr, br, zr, "Quads", CGNS_ENUMV(QUAD_4),
                              1, 1, 0, conn, &secr) ||
            cg_family_write (fr, br, "RindFam", &famr) ||
            cg_goto (fr, br, "Zone_t", zr, NULL) ||
            cg_famname_write ("RindFam") ||
        ...
        /* The offending node.  It must carry a non-zero extent: the library does
         * not emit a Rind_t whose planes are all zero, so a zero rind would leave
         * nothing in the file to detect.  cg_field_write adds rind_planes to the
         * high-order length, so the buffer is sized 9 + 1 to match what the
         * library writes -- 9 degrees of freedom for one QUAD_4 at degree 2, plus
         * the one rind plane. */
        rind[0] = 1; rind[1] = 0;
        ...
        if (cg_field_write (fr, br, zr, sr, CGNS_ENUMV(RealDouble),
                            "Density", dens, &fld))
            cg_error_exit ();
        cg_close (fr);
    }
